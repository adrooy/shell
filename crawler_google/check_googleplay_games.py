...
#    logger.debug("has %d games in db"  % len(games_in_db))
    ...
    company_urls = {}
    with open('zhongdianchangshangnew.csv') as files:
        for line in files:
            line_info = line.strip().split(',', 1)
            url = line_info[1]
            company = line_info[0]
            company_urls[url] = company
    #try:
    if 1==1:
...
#        for company in COMPANY:
        if 1==1:
            ...
            if 1==1:
    #        try:
                'https://play.google.com/store/apps/dev?id=7242995029088679351'
                ...
                company_url = 'https://play.google.com/store/apps/developer?id=Gameloft'
                company_url = 'https://play.google.com/store/apps/dev?id=6258770168633898802'

                if 'https://play.google.com/store/apps/dev?id=' in company_url:
                    '新页面'
                    ...
                    for pkg_name in pkg_names:
                        print pkg_name
                if 'https://play.google.com/store/apps/developer?id=' in company_url:
                    '旧页面'
                    for i in range(1, 100):
                        ...
                        content = google.get_game_list(company_url, data)
                    ...
                    if 1==1:
                        if detail:
                            ...
                            try:
                                ...
                                file_size = ''
                            game_name = soup.find("div", {"class": "info-box-top"}).find("h1", {"itemprop": "name", "class": "document-title"}).find("div").string
                            a = soup.find("div", {"itemprop": "datePublished"})
                            try:
                                y = a.string.split('年')[0]
                                m = a.string.split('年')[1].split('月')[0]
                                d = a.string.split('年')[1].split('月')[1].split('日')[0]
                                update_time = ('%s%s%s' % (y.zfill(4), m.zfill(2), d.zfill(2))).encode('utf-8')
                            except Exception as e:
                                y = a.string.encode('utf-8').decode('gbk').split('暮拧麓')[0]
                                m = a.string.encode('utf-8').decode('gbk').split('暮拧麓')[1].split('膰&oelig;&circ;')[0]
                                d = a.string.encode('utf-8').decode('gbk').split('暮拧麓')[1].split('膰&oelig;&circ;')[1].split('膰&mdash;慕')[0]
                            ...
                            #print area, company, pkg_name, file_size, update_time, game_name
                            '''
